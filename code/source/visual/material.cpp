	...
	if (load || getResourceState() == State::Uninit){
		...
		for (uint32 i=0; i<tex_names.size(); ++i){
			...
			if (tex_names[i].empty()){
				...
				textures[i]= &resources::gCache->getResource<Texture>(tex_names[i]);
			}
		}
...
void Material::createErrorResource(){
	...
	textures[TexType_Color]= &resources::gCache->getErrorResource<Texture>();
}
...
void Material::changeTexture(TexType type, const util::Str8& resname){
	if (resname.empty())
		...
		setTexture(type, &resources::gCache->getResource<Texture>(resname));
}
