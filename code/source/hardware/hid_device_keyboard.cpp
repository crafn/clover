...
KeyboardHidDevice::KeyboardHidDevice(const ConstructInfo& info)
	...
	glfwSetCharCallback(&hardware::gDevice->getWindow(), characterCallback);
	glfwSetKeyCallback(&hardware::gDevice->getWindow(), keyCallback);
}
...
void KeyboardHidDevice::update(){
	...
	for (SizeType i= 0; i < keyCount; ++i){
		int32 key_state= glfwGetKey(&hardware::gDevice->getWindow(), keyList[i].glfwCode);
